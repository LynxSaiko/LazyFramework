...
class LazyFramework:
    ...
    def display_module_info(self, inst: ModuleInstance):
        ...
        table = Table(show_header=True, header_style="bold red", box=box.MINIMAL_DOUBLE_HEAD)
    ...
    def cmd_options(self, args):
        ...
        if hasattr(self.loaded_module.module, "OPTIONS"):
            ...
            table = Table(title=f"Options for {self.loaded_module.name}", box=box.SIMPLE, expand=False)
            ...
            panel = Panel(table, title=f"Options for {self.loaded_module.name}", border_style="white", expand=True)
            console.print(panel)
        else:
    ...
    def cmd_search(self, args):
        ...
        table.add_column("Module", style="bold red", overflow="ellipsis")
        table.add_column("Description", overflow="ellipsis")
        for key, desc in sorted(results):
            table.add_row(key, desc or "(no description)")

        panel = Panel(table, title=f"WAR GAMES", border_style="white", expand=True, width=150)
        console.print(panel)
